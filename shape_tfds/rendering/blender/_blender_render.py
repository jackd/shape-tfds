...
def main(
    ...
    invariants, depthFileOutput, normalFileOutput, albedoFileOutput = setup(depth_scale)
...
def get_args():
    ...
    parser.add_argument(
        "--camera_positions", type=str, help="path to camera_positions npy file"
    )
    parser.add_argument(
        "--resolution", "-r", type=int, nargs=2, default=(128, 128), help="(ry, rx)"
    )
    parser.add_argument("--scale", type=float, default=1.0, help="scale")
    parser.add_argument("--depth_scale", type=float, default=1.4, help="depth scale")
    parser.add_argument("--remove_doubles", type=bool, default=False)
    parser.add_argument("--edge_split", type=bool, default=False)
    parser.add_argument(
        "--fov", "-f", default=30.0, type=float, help="field of view in degrees"
    )
    parser.add_argument(
        "--albedo", "-a", action="store_true", help="whether or not to save albedo"
    )
    parser.add_argument(
        "--depth", "-d", action="store_true", help="whether or not to save depth map"
    )
    parser.add_argument(
        "--normals", "-n", action="store_true", help="whether or not to save normals"
    )
    parser.add_argument(
        "--filename_format",
