...
def strip_zeros(example_data):
    ...
    (
        stripped,
        padding,
    ) = trimesh.voxel.encoding.DenseEncoding(  # pylint: disable=unpacking-non-sequence
        example_data
    ).stripped
...
class PaddedTensor(features.FeatureConnector):
    def __init__(
        self, stripped_feature, pad_kwargs=None, strip_fn=strip_zeros, padded_shape=None
    ):
        ...
        self._padding = features.feature.Tensor(
        ...
        if padded_shape is not None and len(padded_shape) != self._num_dims:
            raise ValueError("padded_shape and stripped_feature shapes inconsistent.")
