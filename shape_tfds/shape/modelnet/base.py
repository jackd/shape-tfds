...
from shape_tfds.core.random import get_random_state, random_context
...
def get_class_names_path(num_classes):
    return os.path.join(os.path.dirname(__file__), "class_names%d.txt" % num_classes)
...
def load_class_freq(num_classes=40):
    ...
    if num_classes == 40:
        ...
        raise ValueError("num_classes must be 10 or 40, got {}".format(num_classes))
...
class CloudConfig(ModelnetConfig):
    ...
    def feature_item(self):
        return (
            "positions",
            tfds.features.Tensor(shape=(self.num_points, 3), dtype=tf.float32),
        )
...
class CloudNormalConfig(ModelnetConfig):
    def __init__(self, num_points, name=None, version="0.0.1", **kwargs):
        ...
        super(CloudNormalConfig, self).__init__(name=name, version=version, **kwargs)
...
class UniformDensityCloudNormalConfig(CloudNormalConfig):
    ...
    def __init__(self, num_points, k=20, r0=0.1, name=None, **kwargs):
        if name is None:
            name = "uniform_density_cloud_normals-%d-%d-%d" % (k, 100 * r0, num_points)
        self._k = k
        self._r0 = r0
