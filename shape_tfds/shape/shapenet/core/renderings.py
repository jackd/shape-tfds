        ...
        if not overwrite:
            model_ids = [
                m for m in model_ids if not all(
                    m in cache for cache in cache_mappings)]
        if len(model_ids) == 0:
            return
...
class BlenderRenderer(Renderer):
    ...
    def base_mapping(self, synset_id, dl_manager=None):
        yield base.extracted_mesh_paths(
            synset_id, dl_manager=dl_manager)
    ...
    def lazy_mapping(self, dl_manager=None):
        ...
        def map_item(key, base_value):
            ...
            image = self._renderer.render(
                base_value, camera_positions)[0]
            return dict(image=image)

        with self._renderer.base_mapping(
                self.synset_id, dl_manager) as base_mapping:
