...
class Renderer(object):
    ...
    def named(name, resolution):
        return {"blender": BlenderRenderer, "trimesh": TrimeshRenderer,}[name](
    ...
    def create_multi_cache(self, synset_id, seeds, dl_manager=None, overwrite=False):
        ...
        if not overwrite:
            model_ids = [
                m for m in model_ids if not all(m in cache for cache in cache_mappings)
            ]
        if len(model_ids) == 0:
            return
            ...
            for model_id in tqdm.tqdm(model_ids):
                ...
                try:
                    renderings = self.render(
                        base_map[model_id], camera_positions=camera_positions
                    )
...
class RenderingConfig(base.ShapenetCoreConfig):
    ...
    def lazy_mapping(self, dl_manager=None):
        ...
        with self._renderer.base_mapping(self.synset_id, dl_manager) as base_mapping:
    ...
    def cache_mapping(self, cache_dir, mode="r"):
        yield mapping.MappedMapping(
            self._cache_mapping(cache_dir), lambda image: dict(image=image)
        )
    ...
    def create_cache(self, cache_dir, dl_manager=None, overwrite=False):
        ...
        self._renderer.create_multi_cache(
            self.synset_id, seeds=(self.seed,), dl_manager=None, overwrite=False
        )
