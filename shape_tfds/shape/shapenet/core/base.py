...
from shape_tfds.core.collection_utils.mapping import Mapping
from shape_tfds.core.downloads import get_dl_manager

_bad_ids = {
    "04090263": ("4a32519f44dc84aabafe26e2eb69ebf4",),  # empty
    # '02958343': (
...
def extracted_mesh_paths(synset_id, dl_manager=None):
    ...
    return Mapping.mapped(
        tuple(model_ids), lambda key: os.path.join(synset_dir, key, "model.obj")
    )
...
def _load_splits_ids(path, excluded):
    ...
    with tf.io.gfile.GFile(path, "r") as fp:
        ...
        for line in fp.readlines():
            ...
            if line == "":
                pass
            record_id, synset_id, sub_synset_id, model_id, split = line.split(",")
            if excluded and model_id in excluded.get(synset_id, ()):
                continue
            del record_id, sub_synset_id
            split_dicts.setdefault(synset_id, {}).setdefault(split, []).append(model_id)
...
def load_split_ids(dl_manager=None, excluded="bad_ids"):
    ...
    return _load_splits_ids(
        dl_manager.download(SPLIT_URL), None if excluded is None else id_sets[excluded]
    )
...
def cloud_loader_context(synset_id, num_points, dl_manager=None):
    ...
    def map_fn(key, value):
        ...
        return sample_faces(value.vertices, value.faces, num_points).astype(np.float32)
