...
class PartnetConfig(tfds.core.BuilderConfig):
    def __init__(self, object_category):
        ...
        super(PartnetConfig, self).__init__(
...
class Partnet(tfds.core.GeneratorBasedBuilder):
    ...
    def label_index_maps(self):
        ...
        return {k: label_index_map(top_names, class_names[k]) for k in self.levels[:-1]}
    ...
    def _generate_examples(self, archive_fn, split, label_index_maps):
        ...
        for path, fp in archive:
            ...
            if (
                len(split_path) < 3
                or not split_path[1].startswith(camel)
                or not (
                    split_path[2].endswith(".h5") and split_path[2].startswith(split)
                )
            ):
                continue
    ...
    def _info(self):
        ...
        positions = tfds.core.features.Tensor(dtype=tf.float32, shape=(10000, 3))
