...
class Variant(object):
    ...
    def validate(cls, x):
        if x not in cls.all():
            raise ValueError('Invalid {}: must be in {}'.format(
                cls.__name__, cls.all()))
...
class ScanObjectNNConfig(tfds.core.BuilderConfig):
    ...
    def __init__(self, variant=Variant.OBJ, background=True, split_key='main'):
        ...
        if split_key == 'main':
            ...
            raise ValueError(
                'split must be one of ("main", 1, 2, 3, 4), got {}'.format(
                    split_key))
        if not background:
            subdir = '{}_nobg'.format(subdir)
        self.subdir = subdir
        name = '{}-{}'.format(variant, subdir)
        super(ScanObjectNNConfig,
              self).__init__(name=name, version=tfds.core.Version('0.0.1'))
    ...
    def _info(self):
        features = tfds.core.features.FeaturesDict(
            dict(positions=tfds.core.features.Tensor(shape=(NUM_POINTS, 3),
                 ...
                 mask=tfds.core.features.Tensor(shape=(NUM_POINTS,),
                                                dtype=tf.bool)))
    ...
    def _split_generators(self, dl_manager):
        ...
        return [
            tfds.core.SplitGenerator(name=split,
                                     gen_kwargs=dict(
                                         path=path,
                                         filename=config.data_path(split)))
    ...
    def _generate_examples(self, path, filename):
        ...
        with tf.io.gfile.GFile(path, 'rb') as fp:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                n = h5['data'].shape[0]
                for i in range(n):
                    yield i, dict(positions=positions[i],
                                  label=labels[i],
                                  mask=masks[i] != -1)
