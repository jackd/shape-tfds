...
import pyrender  # pylint: disable=unresolved-import
...
def vis(model_id, mesh_or_scene):
    ...
    light = trimesh.scene.lighting.SpotLight(
        color=0.5 * np.ones(3), intensity=0.0, innerConeAngle=np.pi / 16.0
    )
    ...
    light = pyrender.SpotLight(
        color=np.ones(3), intensity=10.0, innerConeAngle=np.pi / 16.0
    )
