...
def main(_):
    ...
    with base.zipped_mesh_loader_context(synset_id, dl_manager=dl_manager) as loader:
        for example_id in example_ids:
            try:
                mesh_or_scene = loader[example_id]
                logging.info("Sucessfully loaded {}/{}".format(synset_id, example_id))
                if flags.FLAGS.counts:
                    ...
                    ng = len(geoms)
                    logging.info(
                        "{} geometries, {} vertices, {} faces".format(ng, nv, nf)
                    )
                if flags.FLAGS.show:
                    mesh_or_scene.show()
            except Exception:
                logging.info("Failed to load model {}/{}".format(synset_id, example_id))
                raise
